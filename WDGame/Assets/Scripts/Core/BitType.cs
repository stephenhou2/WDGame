...
namespace GameEngine
{
    ...
    public class BitType
    {
        ...
        {
            Log.Assert(index >= 0 && index < CoreDefine.BitTypeMaxSize, "BitType index out of range!");
            Log.Assert(!string.IsNullOrEmpty(name), "BitType name empty!");
            ...
            {
                Log.Error(ErrorLevel.Fatal, "BiTypeBuffer Create failed,index out of range,event name:{0}", name);
            }
        }
