...
public partial class ResourceMgr : Singleton<ResourceMgr>
{
    ...
    public static T Load<T>(string url, string reason) where T : UnityEngine.Object
    {
        return LoadRes<T>(url, true, reason);
    }

    public static T InstantiateResource<T>(string url, string reason) where T : UnityEngine.Object
    {
    ...
    public static T ResourcesLoad<T>(string url) where T : UnityEngine.Object
    {
        return Resources.Load<T>(url);
    }

    public static Sprite LoadSprite(string url, string reason)
    {
#if UNITY_EDITOR
    ...
    /// <returns></returns>
    public static Sprite LoadSpriteWithoutNullSpriteError(string url, string reason)
    {
        if (string.IsNullOrEmpty(url))
        {
            Log.Error("Load Empty sprite! reason:{0}", reason);
    ...
    public static Texture LoadTexture(string url, string reason)
    {
#if UNITY_EDITOR
        if (mResLoadHandler != null)
        {
    ...
    public static T LoadRes<T>(string url, bool logError, string reason) where T : UnityEngine.Object
    {
#if UNITY_EDITOR
        if (mResLoadHandler != null)
        {
        ...
        if (BuildSetting.ReleaseMode)
        {
            ...
            if (logError)
            {
                Log.Warning(string.Format("[AB]LoadFail[{0}], reason={1}, ret={2}", url, reason, ret));
            }
        }
        else
        {
            ...
            if (logError)
            {
                // Log.Error(string.Format("[Editor]Load Fail[{0}],load reason:{1}", url, reason));
            }
        }
        return null;
    }

    public static T LoadPreloadRes<T>(string url, bool logError, string reason) where T : UnityEngine.Object
    {
        if (BuildSetting.ReleaseMode)
        {
    ...
    public static bool AsyncLoadRes<T>(string url, string reason, FuncAsyncLoadTask callback) where T : UnityEngine.Object
    {
#if UNITY_EDITOR
        if (mResLoadHandler != null)
        {
...
#if UNITY_EDITOR
    private static IResLoadHandler mResLoadHandler;

    public static void InstallResLoadHandler(IResLoadHandler handler)
    {
