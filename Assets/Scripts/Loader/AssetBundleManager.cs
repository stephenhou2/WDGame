...
public class AssetBundleManager : MonoBehaviour
{
    ...
    public bool AsyncLoadAsset(string assetPathSource, FuncAsyncLoadTask completeCallback)
    {
        ...
        {
            mAsyncLoader.Load(assetPath, ab, (Object obj) => {
    ...
    public (int ret, T obj) LoadPreloadAsset<T>(string assetPathSource) where T : UnityEngine.Object
    {
        ...
        {
            var obj = ab.LoadAsset(assetPath);
            var ret = obj as T;
    ...
    /// <returns>包名</returns>
    static private string GetBundleName(string assetPath)
    {
        var index = assetPath.IndexOf("/");
        if (index > 0)
        {
            //特殊说明：船只高低模 特殊处理
            var bundleName = assetPath.Substring(0, index);
            return bundleName;
        }
        return string.Empty;
    }
